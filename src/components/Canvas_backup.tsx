...
export const Canvas: React.FC = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (toolMap[e.key]) {
        useEditorStore.getState().setTool(toolMap[e.key] as any);
      }
    };
  ...
  const handleStageDrag = (e: Konva.KonvaEventObject<DragEvent>, isStart: boolean) => {
    if (tool === 'hand') {
      const cursor = isStart ? 'grabbing' : 'grab';
      stageRef.current?.container().style.setProperty('cursor', cursor);
      if (!isStart) setPanOffset({ x: e.target.x(), y: e.target.y() });
    }
  };
  ...
  const handleDragEvent = (e: React.DragEvent, type: 'over' | 'leave') => {
    e.preventDefault();
  ...
  return (
    ...
    >
      ...
      <Stage
        ...
        draggable={tool === 'hand'}
        onDragStart={() => handleStageDrag({} as any, true)}
        onDragEnd={(e) => handleStageDrag(e, false)}
      >
        <Layer>
          {drawGrid()}
          ...
          {/* Built-in connection handles on all shapes */}
          {elements.filter(el => el.type === 'shape').map(element => {
            const handles = [
              { x: element.x + element.width / 2, y: element.y, anchor: 'top', position: 'top' },
              { x: element.x + element.width, y: element.y + element.height / 2, anchor: 'right', position: 'right' },
              { x: element.x + element.width / 2, y: element.y + element.height, anchor: 'bottom', position: 'bottom' },
              { x: element.x, y: element.y + element.height / 2, anchor: 'left', position: 'left' },
            ];
            ...
            return handles.map((handle, index) => (
              <Circle
                ...
                onDragStart={() => {
                  ...
                  startConnection(element.id, handle.x, handle.y);
                }}
                onDragMove={() => {
                ...
                onDragEnd={() => {
                  // Check if we're over another shape to complete connection
                  const pos = getPointerPosition();
                  ...
                  if (targetElement) {
                    finishConnection(targetElement.id);
                  } else {
                    cancelConnection();
                  }
                }}
